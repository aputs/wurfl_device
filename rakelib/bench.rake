# Some tasks to automate running the WurflDevice Benchmark Suite (WBS)
...
WBS_DIR         = BASEDIR + "/benchmark/wurfl_device"
...
desc "Run the WBS benchmarks"
task :bench => 'bench:run'

namespace :bench do
  desc "Plot the WBS benchmark results"
  ...
  task :run => :setup do
    puts "Running WurflDevice Benchmark Suite"
    puts "  Writing report to #{report_name}"

    Dir[WBS_DIR + "/**/bm_*.rb"].sort.each do |name|
